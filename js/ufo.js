...
// All meshes except for terrain and sky
let meshes = [];
...
let m = {};
const houseColors = {
    darkblue: 0x3630a6, white: 0xffffff, orange: 0xe7751c, black: 0x000000, greyish: 0xd6c3c3, darkbrown: 0x53380e,
...
function createMaterials() {
    ...
    for (const [, color] of Object.entries(houseColors))
        houseM.push(new THREE.MeshPhongMaterial({ color: color }));
...
function updateMaterials(type) {
    ...
    for (let mesh of meshes) {
        let c = mesh.material.color, e = mesh.material.emissive, m = mesh.material.map;
        let o = mesh.material.opacity, t = mesh.material.transparent, w = mesh.material.wireframe;
        if (mesh === moonMesh)
            e = new THREE.Color(colors.lightyellow);
        switch (type) {
            case 'lambert':
...
function createUFO(x, y, z) {
    ...
    meshes.push(createMesh('ell', [d.passengerR, 16, 16, 1, 5/6, 1], passengerMaterial, cockpit3D, 0, 7*d.passengerR/2, 0));
...
function createHouse(x, y, z) {
    ...
    // House 3D (parent: scene; children: blue stripes, walls, roof, windows, door, porch, pillars)
    house3D = createObject3D(ufoScene, x, y, z); 
    let vertices3D = [];
    let indexes3D = [];

    let u = 80; // Settle a unit for simplification purposes
    ...
    const blueStripesIndexes = [0,2,1, 2,0,3, 1,4,5, 5,4,6, 4,7,8, 8,7,9, 10,8,11, 12,8,10, 11,13,14, 14,13,15,
                                17,16,18, 17,18,19, 18,20,22, 20,21,22, 21,23,24, 24,23,25, 25,26,27, 26,17,27,
                                29,28,30, 29,30,31, 30,32,34, 32,33,34, 33,35,36, 36,35,37, 37,38,39, 39,38,29,
                                3,0,40, 40,41,3, 14,15,42, 42,15,43, 44,45,46, 46,45,47, 48,46,50, 49,48,50, 51,49,52,
                                51,52,53, 54,53,55, 45,54,55, 57,56,58, 57,58,59, 58,60,62, 60,61,62, 61,63,64,
                                64,63,65, 65,66,67, 66,57,67, 41,40,42, 41,42,43, 68,69,70, 70,69,71, 72,70,74,
                                73,72,74, 75,73,76, 75,76,77, 78,77,79, 69,78,79, 80,81,82, 82,81,83, 84,82,86,
                                85,84,86, 87,85,88, 87,88,89, 90,89,91, 81,90,91, 92,93,94, 94,93,95, 96,94,98,
    ...
    const wallsIndexes = [0,1,2, 2,1,3, 4,3,5, 4,5,6, 6,7,9, 8,6,9, 10,9,11, 9,2,11, 13,12,14, 13,14,15, 16,5,18,
                          5,17,18, 19,17,21, 17,20,21, 23,22,24, 23,24,25, 26,23,27, 26,27,28, 25,29,30, 30,29,31,
                          28,32,33, 31,28,33, 35,34,36, 35,36,37, 39,38,40, 39,40,41, 42,0,22, 42,22,43, 1,42,44,
                          1,44,45, 43,26,46, 46,26,47, 1,26,48, 21,49,24, 24,49,50, 49,20,51, 51,20,52, 29,50,53,
                          29,53,54, 29,20,55];
    ...
    const roofIndexes = [1,0,2, 1,2,3, 3,2,4, 3,4,5, 6,7,8, 8,7,9, 8,9,10, 10,9,11, 5,4,10, 5,10,11, 7,1,3, 3,9,7,
                         9,3,5, 5,11,9, 0,1,12, 14,12,1, 7,6,13, 13,15,7, 12,14,13, 13,14,15, 12,0,6, 6,12,13, 14,1,7,
                         7,15,14];
    ...
    const windowFrameIndexes = [1,0,2, 1,2,3, 4,5,6, 6,5,7, 9,8,10, 9,10,11, 12,13,14, 14,13,15, 16,17,18, 18,17,19,
                                21,20,22, 21,22,23, 24,25,26, 26,25,27, 29,28,30, 29,30,31, 32,33,34, 34,33,35,
                                37,36,38, 37,38,39, 40,41,42, 42,41,43, 45,44,46, 45,46,47, 49,48,50, 49,50,51,
                                52,53,54, 54,53,55];
    ...
    const windowGlassIndexes = [1,0,2, 1,2,3, 5,4,6, 5,6,7, 9,8,10, 9,10,11, 13,12,14, 13,14,15, 17,16,18, 17,18,19,
                                21,20,22, 21,22,23, 25,24,26, 25,26,27, 29,28,30, 29,30,31, 32,33,34, 34,33,35,
                                36,37,38, 38,37,39, 40,41,42, 42,41,43, 44,45,46, 46,45,47, 48,49,50, 50,49,51,
                                52,53,54, 54,53,55, 56,57,58, 58,57,59, 60,61,62, 62,61,63, 64,65,66, 66,65,67,
                                68,69,70, 70,69,71, 72,73,74, 74,73,75, 76,77,78, 78,77,79, 80,81,82, 82,81,83,
                                84,85,86, 86,85,87, 88,89,90, 90,89,91, 92,93,94, 94,93,95, 97,96,98, 97,98,99,
                                101,100,102, 101,102,103, 105,104,106, 105,106,107, 109,108,110, 109,110,111];
    ...
    const porchIndexes = [3,1,0, 0,2,3, 1,5,0, 4,0,5, 7,3,2, 2,6,7, 5,7,4, 6,4,7, 2,0,8, 2,8,9, 4,6,10, 10,6,11,
                          12,8,13, 10,13,8, 9,14,15, 15,11,9];
    ...
    const pillarsIndexes = [1,0,5, 4,5,0, 2,3,7, 7,6,2, 5,4,7, 6,7,4, 1,5,3, 7,3,5, 9,8,11, 10,11,8, 12,8,9, 9,13,12,
                            10,14,11, 15,11,14, 12,13,15, 15,14,12, 17,16,19, 18,19,16, 20,16,17, 17,21,20, 18,22,19,
                            23,19,22, 20,21,23, 23,22,20, 25,24,27, 26,27,24, 28,24,25, 25,29,28, 26,30,27, 31,27,30,
                            28,29,31, 31,30,28, 33,32,35, 34,35,32, 36,34,33, 33,37,36, 34,38,35, 39,35,38, 36,37,39,
                            39,38,36];
    ...
    for (let i = 0; i < vertices3D.length; i++) {
        ...
        meshes.push(createMesh('buf', [vertices, indexes], houseM[i], house3D));
    }
} 
...
function createMoon(x, y, z) {
    ...
    moonMesh = createMesh('sph', [d.moonR, 16, 16], m.lightyellow, moon3D, d.moonR, d.moonR, d.moonR);
...
function update(delta) {
    ...
    for (let k in keys) {
        if (keys[k] === true) {
            switch (k) {
                // Texture Generation (keys 1, 2)
                case '49': // 1
                    debug('1 - generate new terrain texture');
                    const terrainTexture = new THREE.CanvasTexture(generateTerrainTexture(fieldTextureSize));
                    terrain.material.map = terrainTexture;
                    keys[k] = false;
                    break;
                case '50': // 2
                    debug('2 - generate new sky texture');
                    const skyTexture = new THREE.CanvasTexture(generateSkyTexture(skyTextureSize));
                    skydome.material.map = skyTexture;
                    keys[k] = false;
                    break;
                // Materials Update (keys Q, W, E, R)
                case '81': // Q
                case '113': // q
                    debug('Q - change materials to lambert');
                    updateMaterials('lambert');
                    keys[k] = false;
                    break;
                case '87': // W
                case '119': // w
                    debug('W - change materials to phong');
                    updateMaterials('phong');
                    keys[k] = false;
                    break;
                case '69': // E
                case '101': // e
                    debug('E - change materials to toon');
                    updateMaterials('toon');
                    keys[k] = false;
                    break;
                case '82': // R
                case '114': // r
                    debug('R - change materials to basic');
                    updateMaterials('basic');
                    keys[k] = false;
                    break;
                // Lights Toggling (keys D, P, S)
                case '68': // D
                case '100': // d
                    debug('D - toggle moon directional light visibility');
                    moonLight.visible = !moonLight.visible;
                    keys[k] = false;
                    break;
                case '80': // P
                case '112': // p
                    debug('P - toggle cylinder spotlight visibility');
                    spotLight.visible = !spotLight.visible;
                    keys[k] = false;
                    break;
                case '83': // S
                case '115': // s
                    debug('S - toggle bauble point lights visibility');
                ...
                case '67': // C
                case '99': // c
                    debug('C - toggle camera controls');
                    controls.enabled = !controls.enabled;
                    controls.object = camera;
                    controls.update();
                    keys[k] = false;
                    break;
                // Visual Representation (keys 6, 7, 8, 9)
                case '54': // 6
                    debug('6 - toggle wireframe visibility');
                    Object.values(m).forEach(function(material) {
                        material.wireframe = !material.wireframe;
                    });
                    houseM.forEach(function(material) {
                        material.wireframe = !material.wireframe;
                    });
                    keys[k] = false;
                    break;
                case '55': // 7
                    debug('7 - toggle edges visibility');
                    edgesMaterial.visible = !edgesMaterial.visible;
                    keys[k] = false;
                    break;
                case '56': // 8
                    debug('8 - toggle light helper visibility');
                    ...
                    break;
                case '57': // 9
                    debug('9 - toggle axes helper visibility');
                ...
                // Debugging (keys 0, M)
                case '48': // 0
                    debug('0 - display triangles count: ' + ufoRenderer.info.render.triangles);
                    keys[k] = false;
                    break;
                case '77': // M
                case '109': // m
                    debug('M - toggle debug messages mode');
                    debugMode = !debugMode;
                    keys[k] = false;
                    break;
                // UFO Controls (keys left, up, right, down)
                case '37': // left
                case '38': // up
                case '39': // right
                case '40': // down
...
function init() {
    ...
    controls.target.set(0, yInitial, 0);
...
function animate() {
    ...
    ufoRenderer.setAnimationLoop(animate);
